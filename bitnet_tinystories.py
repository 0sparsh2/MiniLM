...
def main():
    ...
    try:
        ...
        log("Cache miss — downloading tokenizer...")
        os.environ.pop("TRANSFORMERS_OFFLINE", None)
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
    vocab_size = len(tokenizer)
    log(f"Vocab Size: {vocab_size}")

    data_path = "tinystories_train.txt"
    if not os.path.exists(data_path):
        import urllib.request
        url = "https://huggingface.co/datasets/roneneldan/TinyStories/resolve/main/TinyStoriesV2-GPT4-train.txt"
        log("Downloading TinyStories raw text (this may take a minute)...")
        # Added headers to bypass some basic bot blocks if needed, HF raw URLs usually work
    ...
    # Read first 50,000 lines to keep it fast but provide ~1.5M tokens
    lines = []
    ...
    for i, line in enumerate(lines):
        ...
        # Add a space between lines conceptually, or let BPE handle it
        if i % 2000 == 0:
